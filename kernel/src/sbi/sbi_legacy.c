...
long sbi_set_timer(uint64_t stime_value) {
	return sbi_call_legacy_1(EID_SET_TIMER, (uint64_t)stime_value);
}

void sbi_console_putchar(int ch) {
	sbi_call_legacy_1(EID_CONSOLE_PUTCHAR, (uint64_t)ch);
}

long sbi_console_getchar(void) {
	return sbi_call_legacy_0(EID_CONSOLE_GETCHAR);
}

long sbi_clear_ipi(void) {
	return sbi_call_legacy_0(EID_CLEAR_IPI);
}

long sbi_send_ipi(const unsigned long *hart_mask) {
	return sbi_call_legacy_1(EID_SEND_IPI, (uint64_t)hart_mask);
}

long sbi_remote_fence_i(const unsigned long *hart_mask) {
	return sbi_call_legacy_1(EID_REMOTE_FENCE_I, (uint64_t)hart_mask);
}

long sbi_remote_sfence_vma(const unsigned long *hart_mask,unsigned long start, unsigned long size) {
	return sbi_call_legacy_3(EID_REMOTE_SFENCE_VMA, (uint64_t)hart_mask, (uint64_t)start, (uint64_t)size);
}

long sbi_remote_sfence_vma_asid(const unsigned long *hart_mask,unsigned long start, unsigned long size, unsigned long asid) {
	return sbi_call_legacy_4(EID_REMOTE_SFENCE_VMA_ASID, (uint64_t)hart_mask, (uint64_t)start, (uint64_t)size, (uint64_t)asid);
}

void sbi_shutdown(void) {
	sbi_call_legacy_0(EID_SHUTDOWN);
}